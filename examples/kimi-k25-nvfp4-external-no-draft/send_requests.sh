#!/bin/bash
# Send user requests to the external sglang server — Kimi-K2.5 NVFP4
#
...
#
# Prerequisites:
#   - run_trainer.sh must be running on Node 1
#   - run_sglang.sh must be running on Node 2 and healthy
#
# Usage:
#   bash examples/kimi-k25-external-no-draft/send_requests.sh [EXTRA_ARGS...]
#
...
DATASET="${DATASET:-$ROOT_DIR/datasets/onlinesd/merged/merged_train_data.jsonl}"

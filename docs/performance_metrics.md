...
Aurora includes an opt-in performance metrics system designed to identify
pipeline bottlenecks between the asynchronous inference and synchronous training
stages.

## Enabling

Performance metrics are **enabled by default** (`debug.enable_perf_metrics: true`).
To disable them:

```bash
bash examples/qwen3-coder-next-online/run.sh debug.enable_perf_metrics=false
```

When disabled, **zero overhead** is added to the training loop.
...
### Inference Metrics (online training only, aggregated between training steps)

These metrics are only available in **online training** mode (internal SglEngine). In external training mode, inference runs in a separate sglang process outside Aurora's visibility.
...
| `I` | Inference capacity (samples/s) — how fast inference *could* produce if training were infinitely fast. Online training only. |
...
> **Note:** In external training mode, `I` is not available since inference runs
> outside Aurora. Use `dispatch_wait` and `pool` size to infer whether the
> external sglang server is keeping up.

### Example scenarios

**Scenario 1: Inference bottleneck (online training)**
```
...
**Scenario 2: Training bottleneck (online training)**
```
...
**Scenario 3: Inference bottleneck (external training)**
```
thru=5.1, T=15.3, wait=3.5s, pool=0
```
No `I` metric is available. High dispatch wait and empty pool indicate the
external sglang server can't keep up. Consider sending more concurrent requests
(`NUM_WORKERS`), increasing `--max-running-requests` on the sglang server, or
adding more sglang GPUs.

**Scenario 4: Mooncake transfer bottleneck**
```
